 ...
 *
 * Notes about the implementation of the program:
 * This file is for managing the whole hospital system: patients,
 * staffs, care periods, medicines.
 * */
#ifndef HOSPITAL_HH
#define HOSPITAL_HH
...
private:
    ...
    std::map<std::string, Person*> current_patients_; // including only patients in the hospital at the moment

    // Container of all the staff in the hospital
    std::map<std::string, Person*> staff_;

    std::map<std::string, Person*> all_patients_; // including also patients that have left

    // Container of all the care periods of a patient
    //       patient's id   patient's periods (closed and ongoing)
    std::map<std::string, std::vector<CarePeriod*>> care_periods_;

    // Container of all the medicines used in hospital
    //       medicine's name   patients who take the medicine
    std::map<std::string, std::set<string>> all_medicines_;

    // Container of the ids of all patients that have been to hospital
    std::vector<std::string> all_patient_ids_;
};
