...
bool execute_command(const vector<string>& command_line)
{
    string command_to_be_executed = command_line.at(0);
    for ( auto& letter : command_to_be_executed)
    {
        letter=toupper(letter); // convert lowercase to uppercase
    ...
    for ( const Command& command : COMMANDS )
    {
        ...
        if ( command.parameter_number != command_line.size() - 1)
        {
        ...
        if ( not string_to_double(command_line.at(1), left_operand) or
             not string_to_double(command_line.at(2), right_operand))
        {
